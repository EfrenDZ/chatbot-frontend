...
// 1. Add aiSubTab state and updateAiKnowledge helper
                          ...
                          {(formData.aiKnowledge?.catalog || []).map((item, idx) => (
                            <div key={idx} className="bg-gray-50 p-3 rounded border border-gray-200 relative">
                              ...
                              <div className="grid grid-cols-2 gap-3 pr-6">
                                <div><label className="text-[10px] uppercase font-bold text-gray-500">Nombre</label><input type="text" value={item.name} onChange={(e) => { const c = [...formData.aiKnowledge.catalog]; c[idx].name = e.target.value; updateAiKnowledge('catalog', c); }} className="w-full border rounded p-1.5 text-sm"/></div>
                                <div><label className="text-[10px] uppercase font-bold text-gray-500">Precio</label><input type="text" value={item.price} onChange={(e) => { const c = [...formData.aiKnowledge.catalog]; c[idx].price = e.target.value; updateAiKnowledge('catalog', c); }} placeholder="Ej: $50 USD" className="w-full border rounded p-1.5 text-sm"/></div>
...
// Replace the old div
const oldSectionMatch = /<div className="grid grid-cols-1 gap-5">[\s\S]*?<\/div>\n              <\/div>\n/g;
code = code.replace(oldSectionMatch, aiSectionHTML);
...
if(startIdx !== -1) {
  for(let i = startIdx + 1; i < lines.length; i++) {
    if(lines[i].includes('</section>')) { // End of section 3
      endIdx = i;
      break;
    }
  }
